...
To flag pairs, we apply |$Q_3$| > 0.2 critical value ([Chen & Thissen, 1997](https://journals.sagepub.com/doi/abs/10.3102/10769986022003265)) as a first-pass screen, but see the negative-bias caveat below before treating it as a hard cutoff.

```{r}
diag(q3) <- 0 # fix the diagonal to 0, avoiding na error
findCorrelation(q3, cutoff = 0.2, names = TRUE, verbose = TRUE) # flag correlation pairs that is more than |0.2|
mean(q3[upper.tri(q3)]) # calculating mean off-diagonal
```

A couple of things worth knowing before reading this table at face value.

First, $Q_3$ has a well-documented *negative* bias, especially with few items: because $\theta$ is estimated from the same items whose residuals are being correlated, the "no local dependence" baseline isn't 0, it's somewhat below zero ([Yen, 1984, p. 127](https://journals.sagepub.com/doi/abs/10.1177/014662168400800201); [Marais, 2013 in Christensen, et al., 2016](https://journals.sagepub.com/doi/full/10.1177/0146621616677520), p. 121). Here that mean off-diagonal $Q_3$ is -.119.
