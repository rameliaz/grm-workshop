...
Standard CFA assumes continuous items and a strictly linear relationship. When items are **ordinal** or **categorical**, CFA can incorporate thresholds and non-linear link functions (e.g., categorical CFA via `WLSMV`, [see details here](https://lavaan.ugent.be/tutorial/est.html)) which, mathematically, is closely related to polytomous IRT.
:::
...
::: columns
::: {.column width="60%"}
![](../img/diagrams/staircase.png){fig-align="center" width="90%"}
:::
::: {.column width="40%"}
For an item with 5 response categories, GRM estimates **4 thresholds** ($b_1$–$b_4$).

Each threshold is the point on $\theta$ where a participant is equally likely to pick one category or the next, as in a staircase they "step up" as their trait level rises.
...
::: {.column width="40%"}
::: {.incremental}
- The two **extreme** categories ("Never"/"Very often") are cumulative boundary categories, which means their curves plateau rather than peak as in the others.
...
::: {.lead}
Before intrepreting GRM model and item parameters, three core construct assumptions need to hold, which closely mirroring those in factor analysis:
:::
...
## When (Not) to Reach for GRM {.columns-slide}
