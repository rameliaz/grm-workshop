...
* When dealing with a non-significant $M_2$ or $C_2$ test, tentatively reject the model and examine the residual correlations in order to identify any problematic items. 
  - Then decide whether to retain the model in light of these issues.
* At the **item level**, `itemfit()` gives us $S\text{-}X^2$ and its corresponding *p*-value, as well as `RMSEA`.
...
::: {.lead}
Always be cautious of its limitations when using cutoffs/rules of thumb. The use of these cutoffs are disputed in the literature because:
:::
