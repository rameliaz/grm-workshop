...
::: {.lead}
Suppose that we have a Likert-type item with five response categories. Then, we plot the probability of endorsing each category against $\theta$, where each threshold represents the transition boundary between two adjacent categories.
:::
...
::: {.incremental}
* When dealing with a non-significant $M_2$ or $C_2$ test, tentatively reject the model and examine the residual correlations in order to identify any problematic items. Then, decide whether to retain the model in light of these issues.
* At the **item level**, `itemfit()` gives us $S\text{-}X^2$, its *p*-value, and `RMSEA`.
  - **Large** *p*-values of the $S\text{-}X^2$ test **indicate item fit**, and we will see why in the hands-on session.
:::

## Model Fit Parameters {.text-slide-bg}

::: {.lead}
Always be cautious of its limitations when using cutoffs/rules of thumb. 
:::

::: {.incremental}
* Avoid relying on a single index.
* The use of these cutoffs are disputed in the literature because:
  + These cutoffs are tied to specific, simulated model characteristics that don't generalize well when model structures and data conditions change (see [Marsh, et al., 2004](https://www.tandfonline.com/doi/pdf/10.1207/s15328007sem1103_2); [McNeish, 2025](https://www.annualreviews.org/content/journals/10.1146/annurev-psych-121924-104021)).
  + When a continuous, informative statistic gets collapsed into pass/fail, this may discourage researchers from looking at *where* and *how much* misfit exists.
  + Chasing model fit indices may be just as questionable as *p*-value hacking.
:::
...
* **Data:** Teaching extract of the NaDiRA dataset (*N* = `r nrow(data)`, no missing data), with a calibrated simulated option as fallback.
* **Document (two options):** [`exercise.qmd`](files/exercise.qmd?download) or [`exercise.Rmd`](files/exercise.Rmd?download) — open it now in RStudio and follow along.
